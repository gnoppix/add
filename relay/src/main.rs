...
struct MailboxStoreRequest {
    ...
    /// TIER-0 (metadata hardening): when the relay has a shared_secret,
    /// the client sends `recipient_tag = HMAC(secret, recipient_nid || epoch)`
    /// instead of the raw null_id, so the relay never persists the
    /// plaintext recipient identity. The relay prefers this for storage/lookup.
    #[serde(default)]
...
impl RelayState {
    ...
    async fn store_message(&self, req: MailboxStoreRequest) -> Result<(), String> {
        // TIER-0 (metadata hardening): when a shared_secret is configured,
        // key the mailbox by a blind tag HMAC(secret, nid||epoch)
        // instead of the raw null_id, so the plaintext recipient is never
        // persisted. When no secret, fall back to the raw nid (legacy).
        ...
        let tag = recipient_tag(&self.shared_secret, &req.recipient_nid, epoch);
        // The durable mailbox key: tag if present, else raw nid.
        let mailbox_key = tag.clone().unwrap_or_else(|| req.recipient_nid.clone());
        // TIER-1: coarsen the stored timestamp to a bucket so the
        // relay cannot build a fine-grained "who/when" timing graph.
        let now = SystemTime::now()
            .duration_since(UNIX_EPOCH)
            .unwrap_or_default()
            .as_secs();
        let stored_at = (now / ROUTING_BUCKET_SECS) * ROUTING_BUCKET_SECS;

        // Always write to in-memory cache for fast reads
        let mut mailboxes = self.mailboxes.write().await;
        let mailbox = mailboxes
            .entry(mailbox_key.clone())
        ...
        // SECURITY FIX (AUDIT-6): do NOT log message contents or the recipient
        // identity. Log only the (blinded) mailbox key prefix and the length.
        tracing::debug!(
            "relay store: mailbox={}… blob_len={}",
            &mailbox_key.chars().take(8).collect::<String>(),
            req.signed_blob.len(),
        );

        // SECURITY FIX (C5): Persist to SQLite for durability across restarts
        // SECURITY FIX (M3): Encrypt sender metadata (nid + fp) at rest
        // TIER-0: when a tag is used, the plaintext recipient_nid is NOT
        // persisted — only the blind recipient_tag is stored.
        if let Some(ref pool) = self.db_pool {
            ...
            sqlx::query(
                    "INSERT INTO mailbox_entries (recipient_nid, recipient_tag, signed_blob, sender_nid, sender_fp, seq, stored_at, sender_encrypted)
                     VALUES (?, ?, ?, ?, ?, ?, ?, ?)"
                )
                .bind(stored_recipient_nid)
                .bind(&stored_recipient_tag)
                .bind(&req.signed_blob)
                .bind(&req.sender_nid)
                .bind(&req.sender_fp)
                .bind(req.seq)
                .bind(stored_at as i64)
    ...
    async fn fetch_messages(&self, recipient_nid: &str) -> Vec<MailboxEntry> {
        // TIER-0: compute the candidate mailbox keys for this recipient.
        // When a shared_secret is configured, messages are stored under the
        // blind tag HMAC(secret, nid||epoch). Because the tag rotates hourly,
        // we look up both the current and previous epoch to tolerate clock
        // skew and messages that spanned an epoch boundary. The raw nid is
        // always included as a legacy fallback.
        let now = SystemTime::now()
            .duration_since(UNIX_EPOCH)
            .unwrap_or_default()
            .as_secs();
        let epoch = routing_epoch(now);
        let mut keys: Vec<String> = Vec::new();
        if let Some(t) = recipient_tag(&self.shared_secret, recipient_nid, epoch) {
            keys.push(t);
        }
        if epoch > 0 {
            if let Some(t) = recipient_tag(&self.shared_secret, recipient_nid, epoch - 1) {
                keys.push(t);
            }
        }
        keys.push(recipient_nid.to_string());
        ...
        // Fallback to in-memory cache: try each candidate key.
        let mailboxes = self.mailboxes.read().await;
        for k in &keys {
            if let Some(mb) = mailboxes.get(k) {
                let e = mb.fetch_all();
                if !e.is_empty() {
                    return e;
                }
    ...
    /// TIER-0: the candidate durable mailbox keys for a recipient nid —
    /// blind tag for the current + previous epoch (when a shared_secret is
    /// set) plus the raw nid as legacy fallback. Used by fetch/ack/purge so
    /// they all agree on where a recipient's messages live.
    fn mailbox_keys(&self, recipient_nid: &str) -> Vec<String> {
        let now = SystemTime::now()
            .duration_since(UNIX_EPOCH)
            .unwrap_or_default()
            .as_secs();
...
/// TIER-1 (metadata hardening): coarsen absolute timestamps so the relay
/// cannot build a fine-grained "who-talked-when" timing graph.
const ROUTING_BUCKET_SECS: u64 = 60;

/// TIER-0 (metadata hardening): blind the stored recipient. When the relay
/// has a `shared_secret`, the mailbox is keyed by
/// `HMAC(secret, recipient_nid || epoch)` instead of the raw null_id, so
/// the relay never persists the plaintext recipient identity. The per-recipient
/// tag rotates hourly so it is not a permanent stable identifier.
const ROUTING_EPOCH_SECS: u64 = 3600;

fn routing_epoch(now_secs: u64) -> u64 {
    now_secs / ROUTING_EPOCH_SECS
}
...
mod tests {
    ...
    // ---- TIER-0 / TIER-1 metadata-hardening tests ----
    ...
    fn store_req(recipient_nid: &str, sender_nid: &str, blob: &str, tag: &str) -> MailboxStoreRequest {
        MailboxStoreRequest {
            ...
            signed_blob: blob.to_string(),
            sender_nid: sender_nid.to_string(),
            sender_fp: "fp".to_string(),
            seq: 1,
            sender_sig: "sig".to_string(),
            timestamp: 1234567890.0,
    ...
    #[tokio::test]
    async fn test_relay_blinds_recipient_null_id() {
        // With a shared_secret, the relay must store/serve the mailbox under
        // the blind routing tag and MUST NOT persist the plaintext recipient
        // null_id.
        let dir = std::env::temp_dir().join(format!("add-relay-test-{}", uuid_hex()));
        let state = RelayState::new(
            Some("tier0-secret".to_string()),
        ...
        .await
        .expect("new");
        let nid = "NN-RECIPIENT-AAAA-BBBB";
        let tag = recipient_tag(&state.shared_secret, nid, routing_epoch(now_unix() as u64))
            .expect("tag when secret set");

        state
            .store_message(store_req(nid, "NN-SENDER", "enc-blob", &tag))
            .await
            .expect("store ok");
        ...
        assert_eq!(got.len(), 1, "message fetched by nid (relay derives tag)");
        assert_eq!(got[0].signed_blob, "enc-blob");

        // The raw plaintext nid MUST NOT appear in any persisted mailbox row.
        // We assert it is not a key; combined with the store logic that blanks
        // recipient_nid when a tag is present, the plaintext identity never
        // rests on the relay.
        let keys = state.mailbox_keys(nid);
        assert!(
            keys.last() == Some(&nid.to_string()),
            "nid only appears as legacy fallback, never as the primary tag"
        );
    }

    #[tokio::test]
    async fn test_relay_no_secret_keeps_legacy_nid() {
        // Without a shared_secret, blind tags are disabled and the relay
        // falls back to the legacy plaintext-nid keying (backward compat).
        let dir = std::env::temp_dir().join(format!("add-relay-test-{}", uuid_hex()));
        let state = RelayState::new(
            None,
            dir.to_string_lossy().to_string(),
        ...
        .await
        .expect("new");
        let nid = "NN-RECIPIENT-CCCC-DDDD";
        state
            .store_message(store_req(nid, "NN-SENDER", "enc-blob2", ""))
            .await
            .expect("store ok");
        ...
        let got = state.fetch_messages(nid).await;
        assert_eq!(got.len(), 1);
        assert_eq!(got[0].signed_blob, "enc-blob2");
    }

    #[tokio::test]
    async fn test_relay_timestamp_bucketed() {
        // TIER-1: stored_at should be coarsened to ROUTING_BUCKET_SECS so the
        // relay cannot build a fine-grained timing graph.
        let dir = std::env::temp_dir().join(format!("add-relay-test-{}", uuid_hex()));
        let state = RelayState::new(
            None,
            dir.to_string_lossy().to_string(),
        ...
        .await
        .expect("new");
        let nid = "NN-BUCKET-TEST-1111";
        state
            .store_message(store_req(nid, "NN-SENDER", "blob", ""))
            .await
            .expect("store ok");
        let got = state.fetch_messages(nid).await;
        assert_eq!(got.len(), 1);
        assert_eq!(
            got[0].stored_at % ROUTING_BUCKET_SECS,
            0,
