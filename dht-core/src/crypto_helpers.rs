...
/// Syntax check for null ID format: NN-XXXX-XXXX-… (8 groups of 4 hex chars).
/// Also accepts addr:NN-… prefix for address records.
...
/// Derive a Null ID from a GPG fingerprint.
///
/// SECURITY FIX (F-5): was 8-byte Blake2b (~2^48 space) — fully enumerable, so
/// an attacker could precompute KEM keys / probe every address. Now 16-byte
/// Blake2b → 128-bit entropy (32 hex chars), rendered `NN-aaaa-bbbb-…` (8 groups).
/// Still a one-way mapping (fingerprint not recoverable) and still content-
/// addressed by `H(pubkey)` semantics; only the entropy and presentation change.
