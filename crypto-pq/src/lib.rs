...
pub use kem::{
    MlKem1024DecapsulationKey, MlKem1024EncapsulationKey, MlKem1024Keypair, decapsulate,
    encapsulate,
};
pub use keys::{MlDsa87KeyPair, MlKem1024KeyPair, PqKeyPair};
pub use signature::{MlDsa87Signature, MlDsa87SigningKey, MlDsa87VerifyingKey, sign, verify};
...
pub fn decode_verifying_key(bytes: &[u8]) -> Result<MlDsa87VerifyingKey, crate::error::PqError> {
    use ml_dsa::KeyInit;
    MlDsa87VerifyingKey::new_from_slice(bytes).map_err(|_e| {
        crate::error::PqError::InvalidKeyLength {
            expected: 0,
            got: 0,
        }
    })
