  ...
  return (
    ...
    >
      <div className="relative">
        ...
        >
          {(() => {
            const stickers = stickerEmojis(message.content)
            if (stickers) {
              // Pure-emoji message: render as animated stickers, no chat bubble padding.
              return (
                <div className="flex flex-wrap items-end gap-1">
                  {stickers.map((e, i) => (
                    <EmojiImg key={i} emoji={e} size={48} />
                  ))}
                </div>
              )
            }
          ...
          {isImage && (
            <button
              ...
              className="mt-1 block w-full max-w-[240px] overflow-hidden rounded-md"
              title="Click to view"
            >
              <img
          ...
          {att && !isImage && (
            ...
            >
              ...
              <span className="min-w-0 flex-1">
                ...
                {DANGEROUS_EXT.test(att.name) && (
                  <span className="mt-0.5 block font-semibold text-red-500">
                    ⚠ executable file — verify the sender before opening
                  </span>
                )}
              </span>
            </button>
